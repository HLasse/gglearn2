...
#'@title gglearn2: Shiny App for learning ggplot2
#'@description
...
#'
#'@export
gglearn2 <- function(df=iris){
  dataset = df
  # Get named list of columns in dataset
  columns <- setNames(as.list(names(dataset)), names(dataset))

  ui <- shiny::navbarPage("gglearn2", windowTitle = NULL, theme = "lumen",

                   # Tab 1: Intro and flowchart
                   shiny::tabPanel(title = "Introduction", icon = icon("stats", lib = "glyphicon"),
